...
pub fn spec_for(table: &str, expiration_days: Option<u32>) -> TableSpec {
    ...
    let extra: &[Column] = match table {
        ...
        "schemas" => &[Column { name: "object", ty: "STRING" }],
        "lists" => &[
            Column { name: "list_id", ty: "STRING" },
            Column { name: "object", ty: "STRING" },
        ],
        "list_rows" => &[
            Column { name: "list_id", ty: "STRING" },
        ...
        "users" | "workflows" => &[Column { name: "id", ty: "STRING" }],
        "settings" => &[],
        "activities" => &[
            Column { name: "object", ty: "STRING" },
            Column { name: "record_id", ty: "STRING" },
            Column { name: "activity_id", ty: "STRING" },
        ],
        "attachments" => &[
            Column { name: "object", ty: "STRING" },
            Column { name: "record_id", ty: "STRING" },
            Column { name: "attachment_id", ty: "STRING" },
        ],
        "runs" => &[
            Column { name: "finished_at", ty: "TIMESTAMP" },
            Column { name: "status", ty: "STRING" },
            Column { name: "resources", ty: "JSON" },
        ],
        other => unreachable!("unknown table shape: {other}"),
    };
    cols.extend_from_slice(extra);
    if table != "runs" {
        cols.push(Column { name: "data", ty: "JSON" });
    }
    TableSpec {
        name: table.to_string(),
        columns: cols,
        // The runs ledger is tiny and is the completeness record: never expire it.
        partition_expiration_days: if table == "runs" { None } else { expiration_days },
    }
}
...
mod tests {
    ...
    fn schema(slug: &str) -> ObjectSchema {
        ObjectSchema { slug: slug.into(), relationships: vec![], raw: serde_json::json!({}) }
    }
    ...
    fn runs_table_never_expires() {
        assert!(spec_for("runs", Some(400)).partition_expiration_days.is_none());
        assert_eq!(spec_for("records_person", Some(400)).partition_expiration_days, Some(400));
    }
}
